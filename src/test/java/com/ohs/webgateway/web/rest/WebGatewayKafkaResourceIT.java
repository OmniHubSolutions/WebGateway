...
import com.ohs.webgateway.config.KafkaProperties;
...
class WebGatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        WebGatewayKafkaResource kafkaResource = new WebGatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/web-gateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/web-gateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
